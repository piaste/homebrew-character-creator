...
let rec combatMastery : ArchetypeDef = {
    ...
    Name = "Combat Mastery"
    Grants = [ Buff { StatModifiers.Zero with ``Attack rolls`` = 4 } ]
}

let rec chaoticFortune : ArchetypeDef = {
    Id = % nameof chaoticFortune
    Name = "Chaotic Fortune"
    Grants = [ Complex ("Chaotic Fortune", "Each turn in combat, you have a 5% chance to experience a positive surge of Wild Magic.") ]
}

let rec controlledAggro : ArchetypeDef = {
    Id = % nameof controlledAggro
    Name = "Controlled Aggro"
    Grants = [ Complex ("Controlled Aggro", "While in combat, enemies will see you as a priority target.") ]
}

let rec deepenedResolve : ArchetypeDef = {
    Id = % nameof deepenedResolve
    Name = "Deepened Resolve"
    Grants = [ Complex ("Deepened Resolve", "Gain a bonus to your Saving Throws equal to your Proficiency Bonus.") ]
}
...
let rec imposingWill : ArchetypeDef = {
    ...
    Name = "Imposing Will"
    Grants = [ Complex ("Imposing Will", "Gain a +2 bonus to your Difficulty Class when casting Spells or applying conditions.") ]
}

let rec ironBreaker : ArchetypeDef = {
    Id = % nameof ironBreaker
    Name = "Iron Breaker"
    Grants = [ Complex ("Iron Breaker", "When dealing damage, you ignore all Physical Resistance (Bludgeoning, Piercing, Slashing).") ]
}

let rec ironFist : ArchetypeDef = {
    Id = % nameof ironFist
    Name = "Iron Fist"
    Grants = [ Complex ("Iron Fist", "Your Physical Unarmed Damage deals +4 damage.") ]
}

let rec martialBalance : ArchetypeDef = {
    Id = % nameof martialBalance
    Name = "Martial Balance"
    Grants = [ Complex ("Martial Balance", "You add your global Strength Modifier to weapon damage dealt with dual-wielding.") ]
}
...
let rec measuredArcana : ArchetypeDef = {
    ...
    Grants = [ Complex ("Measured Arcana", "Damaging, area of effect Spells that you cast will deal no damage to your allies.") ]
}

let rec quickReflexes : ArchetypeDef = {
    Id = % nameof quickReflexes
    Name = "Quick Reflexes"
    Grants = [ Buff { StatModifiers.Zero with Initiative = 4 } ]
}

let rec reactiveRetribution : ArchetypeDef = {
    Id = % nameof reactiveRetribution
    Name = "Reactive Retribution"
    Grants = [ Complex ("Reactive Retribution", "When an ally is damaged, you may expand your reaction to make a melee-based Attack Roll against their aggressor.") ]
}

let rec restorativeTouch : ArchetypeDef = {
    Id = % nameof restorativeTouch
    Name = "Restorative Touch"
    Grants = [ Complex ("Restorative Touch", "When healing another creature, restore additional Hit Points equal to your proficiency bonus x 2.") ]
}

let rec temperedBody : ArchetypeDef = {
    Id = % nameof temperedBody
    Name = "Tempered Body"
    Grants = [ Complex ("Tempered Body", "Gain Resistance to all Physical Damage (Bludgeoning, Piercing, Slashing).") ]
}

let rec temperedMind : ArchetypeDef = {
    Id = % nameof temperedMind
    Name = "Tempered Mind"
    Grants = [ Complex ("Tempered Mind", "Gain Resistance to all Elemental Damage (Cold, Fire, Lightning, Thunder, Psychic, Radiant, Necrotic, Force, Poison, Acid).") ]
}

let rec uncheckedForce : ArchetypeDef = {
    Id = % nameof uncheckedForce
    Name = "Unchecked Force"
    Grants = [ Complex ("Unchecked Force", "Gain a bonus to your weapon-based damage equal to your Proficiency Bonus") ]
}

let rec weaveBreaker : ArchetypeDef = {
    Id = % nameof weaveBreaker
    Name = "Weave Breaker"
    Grants = [ Complex ("Weave Breaker", "When dealing damage, you ignore all Elemental Resistances (Cold, Fire, Lightning, Thunder, Psychic, Radiant, Necrotic, Force, Poison, Acid).") ]
}

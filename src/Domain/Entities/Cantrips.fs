...
let rec ``Friends`` : CantripDef = {
    Id = % nameof ``Friends``
    CantripDescription = "The Spellcaster who inflicted this Charm will have Advantage on Charisma based Ability Checks against this non-hostile creature. May be cast while Silenced or Muted and will not break Stealth. Duration: permanent."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Guidance`` : CantripDef = {
    Id = % nameof ``Guidance``
    CantripDescription = "Grant affected creature a bonus 1d4 to Ability Checks. May only affect one target at a time. Duration: permanent."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Guiding Moonlight`` : CantripDef = {
    ...
    CantripDescription = "Infuse a weapon granting it a +2 bonus to both it's Attack Rolls and Damage. Weapons may benefit from only one enchantment from Spells or Cantrips. May only affect one weapon at a time. Duration: permanent."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Resistance`` : CantripDef = {
    ...
    CantripDescription = "Grant affected creature a bonus 1d4 to Saving Throws. May only affect one target at a time. Duration: permanent."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Spellbound Empowerment`` : CantripDef = {
    ...
    CantripDescription = "Your melee or ranged weapon(s) use your Spellcasting Ability for Attack and Damage Rolls. This effect persists so long as you can cast the Cantrip. Duration: permanent."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Titan's Boon`` : CantripDef = {
    ...
    CantripDescription = "Enlarge yourself, improving the majority of your physical capabilities and causing you to deal an additional 1d4 Damage with all Melee Weapon and Unarmed attacks. Duration: permanent."
    Concentration = true
    ActionCost = BonusAction
}

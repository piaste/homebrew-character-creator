...
let rec ``Create or Extinguish Flames`` : CantripDef = {
    ...
    CantripDescription = "Create a 3 m flame surface or extinguish fire from a creature, object or surface. Duration: 3 turns."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Dancing Lights`` : CantripDef = {
    ...
    CantripDescription = "Create glowing wisps illuminating a 9 m radius, sneaking or invisible enemies within the radius are revealed. You may also choose to instead cause a creature or item to glow brightly, illuminating a 9 m radius. Duration: until long rest."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Disguise Self`` : CantripDef = {
    ...
    CantripDescription = "Turn yourself invisible until the end of your current turn or magically change all aspects of your appearance until long rest, so long as you can cast the Cantrip. Duration: 1 turn."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Friends`` : CantripDef = {
    Id = % nameof ``Friends``
    CantripDescription = "The Spellcaster who inflicted this Charm will have Advantage on Charisma based Ability Checks against this non-hostile creature. May be cast while Silenced or Muted and will not break Stealth. Duration: until long rest."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Green Flame-Blade`` : CantripDef = {
    ...
    CantripDescription = "Deal 1d8 Fire Damage alongside your weapon damage and ricochet a spark of flame into another enemy within 9m who takes 1d8 Fire Damage. This Cantrip may be cast while Silenced or Muted. Offense: Melee Weapon."
    Concentration = false
    ActionCost = Action
}
...
let rec ``Guidance`` : CantripDef = {
    Id = % nameof ``Guidance``
    CantripDescription = "Grant affected creature a bonus 1d4 to Ability Checks. May only affect one target at a time. Duration: until long rest."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Guiding Moonlight`` : CantripDef = {
    ...
    CantripDescription = "Infuse a weapon granting it a +2 bonus to both it's Attack Rolls and Damage. Weapons may benefit from only one enchantment from Spells or Cantrips. May only affect one weapon at a time. Duration: until long rest."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Mage Hand`` : CantripDef = {
    Id = % nameof ``Mage Hand``
    CantripDescription = "Summons a spectral hand that can do things that hands do. Duration: until long rest."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Mind Sliver`` : CantripDef = {
    ...
    CantripDescription = "Deal 1d8 Psychic Damage to a creature, potentially Fracturing their ability to think clearly. This causes them to suffer a 1d4 penalty to their next Saving Throw alongside not being able to utlize their armours elemental Damage Reduction. Duration: 3 turns. Offense: Ranged 18m."
    Concentration = false
    ActionCost = Action
}
...
let rec ``Mud Mantle`` : CantripDef = {
    ...
    CantripDescription = "Gain Resistance to the next instance of elemental damage you would take and you cannot Burn. Condition removed upon taking damage. This Cantrip may be cast while Silenced or Muted. Duration: 3 turns."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Prestidigitation`` : CantripDef = {
    ...
    CantripDescription = "The next Attack Roll made against the affected creature will be made with Disadvantage. Duration: 3 turns."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Resistance`` : CantripDef = {
    ...
    CantripDescription = "Grant affected creature a bonus 1d4 to Saving Throws. May only affect one target at a time. Duration: until long rest."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Spare the Dying`` : CantripDef = {
    ...
    CantripDescription = "Heal 1d8 to an allies Hit Points from a distance. Cannot target self."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Spark Surge`` : CantripDef = {
    ...
    CantripDescription = "Deal 1d8 Lightning Damage to a target, potentially Shocking them. This causes them to be unable to use Reactions and have Disadvantage on Dexterity based Ability Checks and Saving Throws. Duration: 1 turn. Offense: Ranged 18m."
    Concentration = false
    ActionCost = Action
}
...
let rec ``Spellbound Empowerment`` : CantripDef = {
    ...
    CantripDescription = "Your melee or ranged weapon(s) use your Spellcasting Ability for Attack and Damage Rolls. This effect persists so long as you can cast the Cantrip. Duration: until long rest."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Titan's Boon`` : CantripDef = {
    ...
    CantripDescription = "Enlarge yourself, improving the majority of your physical capabilities and causing you to deal an additional 1d4 Damage with all Melee Weapon and Unarmed attacks. Duration: until long rest."
    Concentration = true
    ActionCost = BonusAction
}
...
let rec ``Thorn Whip`` : CantripDef = {
    ...
    CantripDescription = "Deal 1d8 Piercing Damage and potentially pull a creature 9 m closer to you. Cannot pull targets if their size is classified as Huge. Offense: Ranged 18m."
    Concentration = false
    ActionCost = Action
}
...
let rec ``Thunderous Strike`` : CantripDef = {
    ...
    CantripDescription = "Deal 1d8 Thunder Damage alongside your weapon damage and inflict a condition that causes the target to take an additional 1d8 Thunder Damage if they move. This Cantrip may be cast while Silenced or Muted. Duration: 1 turn. Offense: Melee Weapon."
    Concentration = false
    ActionCost = Action
}
...
let rec ``Torrent`` : CantripDef = {
    Id = % nameof ``Torrent``
    CantripDescription = "Target a creature and cause them to become Wet. This Cantrip may be cast while Silenced or Muted and will not break Stealth. Duration: 3 turns."
    Concentration = false
    ActionCost = BonusAction
}
...
let rec ``Warpstone`` : CantripDef = {
    Id = % nameof ``Warpstone``
    CantripDescription = "While not Threatened, throw a magical stone, immediately warping to it's location. This Cantrip may be cast while Silenced or Muted and will not break Stealth."
    Concentration = false
    ActionCost = BonusAction
}

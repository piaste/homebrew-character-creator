...
let rec charger : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [ 
        Power(BonusAction, OncePerTurn, "GenericIcon_DamageType_Lightning" <!!> "Charge", "Charge towards nearby enemies as a Bonus Action once per turn, Attacking them in the process.")
        Power(BonusAction, OncePerTurn, "GenericIcon_DamageType_Lightning" <!!> "Charge", "Charge towards nearby enemies as a Bonus Action once per turn, Shoving them in the process.")
    ]
}
...
let rec classSpecialist : FeatDef = {
    ...
    ExplicitDescription = Some "Select two class passives from any class passives list. You can pick this feat once per class, including your current class."
...
let rec defensiveDuellist : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        Power(FreeAction, AtWill, "PassiveFeature_Banite_TacticalDiscipline" <!!> "Defensive Duelist", "When an enemy attacks you with a Melee or a Ranged attack, gain Armor Class equal to your proficiency bonus.")
...
let rec duellistsInstinct : FeatDef = {
    ...
    Grants = [
        ...
        Complex("PassiveFeature_MediumArmorMaster" <!!> "Duellist’s Instinct: Charges", "Gain 3 Duellist Charges per Short Rest. Spend to gain Advantage on Attack Rolls, or to force enemies to attack you with Disadvantage.")
        ...
        Complex("PassiveFeature_MediumArmorMaster" <!!> "Duellist’s Instinct: Strength", "Add your global Strength bonus to your main-hand attack while dual wielding.")
    ]
}
...
let rec dunesis : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Dunesis: Careful", "Your AOE Spells do not affect your allies.")
...
let rec elementalAdept : FeatDef = {
    ...
    Grants = 
        ...
        [
           ...
           Complex("{{DMGTYPE}} Adept: Damage", "When you deal damage of your chosen types to an enemy, you deal an additional +1d4 {{DMGTYPE}} damage of that same type.")
        ]
}
...
let rec galvanizedStormsoul : FeatDef = {
    ...
    Grants = [
        ...
        Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Galvanized Stormsoul: Strike", "Additionally, you gain Galvanized Storm Strike, a melee cantrip which deals weapon damage +1d8/2d8/3d8 and casts Witch Bolt at all enemies within 9m of you.")
    ]
}
...
let rec intrinsicBulwark : FeatDef = {
    ...
    Grants = [
        ...
        Complex("Intrinsic Bulwark: Immunity", "In addition, you are immune to effects which deal damage over time (burning, shocked, bleeding, etc.).")
    ]
}
...
let rec sharpshooter : FeatDef = {
    ...
    Grants = [
        ...
        Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Sharpshooter: Aim", "Attack Rolls with ranged weapons do not benefit from your Proficiency Bonus. However, the bonus is added twice when rolling for damage (toggled).")
...
let rec shieldbearer : FeatDef = {
    ...
    Grants = [
        ...
        Complex("PassiveFeature_MultiattackDefense" <!!> "Shieldbearer: Charges", "Gain three Shieldbearer Charges which can be used to negate an instance of incoming damage.")
        Power(FreeAction, AtWill, "PassiveFeature_MultiattackDefense" <!!> "Shieldbearer", "Additionally, you can spend a Shieldbearer charge to reflect projectiles for 1 turn as a Free Reaction after performing a Shield Bash.")
    ]
}
...
let rec warCaster : FeatDef = {
    ...
    Grants = [
        ...
        Power(FreeAction, AtWill, "PassiveFeature_WarCaster_OpportunitySpell" <!!> "War Caster", "Additionally, you can cast Shocking Grasp against an enemy who enters into melee range as a Free Reaction.")
        Power(BonusAction, AtWill, "Bind Weapons", "Bind Weapons: use Spellcasting Ability for Melee Attack/Damage. Once per turn, upon killing another creature or landing a Critical Hit with your Bound Weapon, you restore a level one Spell Slot.")

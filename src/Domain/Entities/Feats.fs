...
let rec alchemist : FeatDef = {
    ...
    Grants = [ 
        ...
        Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Alchemist: Grenades", "Additionally, thrown grenades will trigger an additional explosion, dealing 1d12 damage per 2 character level to all creatures in a 3m radius around your original target.")
        Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Alchemist: Coatings", "Furthermore, upon applying a coating to your weapon, it will persist until your next Long Rest.")
    ]
}
...
let rec athleticism : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Athleticism: Action", "You gain an additional Bonus Action.")
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Athleticism: Jump", "Your jump distance is doubled.")
    ]
}
...
let rec blessingOfTheMoon : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        Complex("Spell_Enchantment_Bless" <!!> "Blessing of the Moon: Cantrips", "Gain 3 cantrips : Blessing of Moonfire (AOE Cantrip, Action, deal 1d12 radiant damage to any nearby foe + inflicts Holy Flame : 1d12 radiant damage until removed), Lunar Flare (Ranged Cantrip, Action, scaling 1d12 radiant damage in a 3m radius on impact + inflicts Holy Flame : 1d12 radiant damage until removed), Lunar Strike (Melee Weapon Cantrip, Action, weapon damage + scaling 1d8 radiant damage + Inflicts Holy Flame: 1d12 radiant damage until removed).")
        Complex("Spell_Enchantment_Bless" <!!> "Blessing of the Moon: Radiance", "In addition, dealing damage to an enemy restores 1d4 Hit Points to all nearby allies around your target.")
    ]
}
...
let rec charger : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [ 
        Power(BonusAction, OncePerTurn, "GenericIcon_DamageType_Lightning" <!!> "Charger: Attack", "Charge toward a nearby enemy, Attacking them in the process.")
        Power(BonusAction, OncePerTurn, "GenericIcon_DamageType_Lightning" <!!> "Charger: Shove", "Charge toward a nearby enemy, Shoving (Push 9m/30ft) them in the process.")
    ]
}
...
let rec composed : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        Power(Reaction, AtWill, "PassiveFeature_Banite_TacticalDiscipline" <!!> "Regain Composure", "Upon taking damage, you may expend your Reaction in order to restore 1d6*your Proficiency Bonus Hit Points to anyone in a 3m radius around yourself.")
        Power(Reaction, AtWill, "PassiveFeature_Banite_TacticalDiscipline" <!!> "Targeted Composure", "Upon dealing damage with an attack roll, you may expend your Reaction in order to restore 1d6*your Proficiency Bonus Hit Points to anyone in a 3m radius around your target.")
    ]
}
...
let rec defensiveDuellist : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        Power(FreeAction, AtWill, "PassiveFeature_Banite_TacticalDiscipline" <!!> "Defensive Duellist: Parry", "When an enemy attacks you with a Melee or a Ranged attack, you can use a Free Reaction to gain AC equal to your Proficiency Bonus.")
        Power(Reaction, AtWill, "PassiveFeature_Banite_TacticalDiscipline" <!!> "Defensive Duellist: Counter", "In addition, you can counterattack with a Melee or a Ranged Attack as a Reaction.")
...
let rec duellistsInstinct : FeatDef = {
    ...
    Grants = [
        ...
        Complex("PassiveFeature_MediumArmorMaster" <!!> "Duellist’s Instinct: Charges", "In addition, you gain three Duellist Charges per Short Rest. These charges can be spent to gain Advantage on Attack Rolls, or to force enemies to attack you with Disadvantage.")
        ...
        Complex("PassiveFeature_MediumArmorMaster" <!!> "Duellist’s Instinct: Opportunity", "Additionally, your Opportunity Attacks hit with both weapons when dual-wielding.")
        Complex("PassiveFeature_MediumArmorMaster" <!!> "Duellist’s Instinct: Strength", "Finally, you add your Global Strength bonus to your main-hand attack while dual wielding.")
    ]
}
...
let rec dunesis : FeatDef = {
    ...
    Grants = [
        ...
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Dunesis: Power", "Spells you cast and Conditions you apply have their Difficulty Class increased by 1. You may further increase this number by consuming Dunesis Charges.")
...
let rec durable : FeatDef = {
    ...
    Grants = [
        ...
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Durable: Unstoppable", "In addition, you have Lockdown Immunity (immune to most Crowd Control effects).")
    ]
}
...
let rec elementalAdept : FeatDef = {
    ...
    Grants = 
        ...
        [
           Complex("{{DMGTYPE}} Adept: Bypass", "When dealing {{DMGTYPE}} damage, you ignore Resistances and Immunities.")
           Complex("{{DMGTYPE}} Adept: Damage", "When you deal {{DMGTYPE}} damage to an enemy, you deal an additional +1d4 {{DMGTYPE}} damage.")
        ]
}
...
let rec galvanizedStormsoul : FeatDef = {
    ...
    Grants = [
        ...
        Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Galvanized Stormsoul: Strike", "Additionally, you gain Galvanized Storm Strike, a melee cantrip which deals weapon damage +1d8/2d8/3d8 (cantrip scaling) and cast Witch Bolt at all enemies within 9m of you.")
    ]
}
...
let rec intrinsicBulwark : FeatDef = {
    ...
    Grants = [
        ...
        Complex("Intrinsic Bulwark: Immunity", "In addition, you are immune to effects which deal damage over time (burning, shocked, bleeding, ...)")
    ]
}
...
let rec mageSlayer : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Mage Slayer: Disruption", "Enemies you damage immediately lose Concentration on any Spells they were maintaining.")
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Mage Slayer: Silence", "Upon dealing damage, you silence the target for one turn.")
    ]
}
...
let rec martialAdept : FeatDef = {
    ...
    Grants = [
        ...
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Martial Adept", "When making a Melee Weapon Attack, Unarmed Attack, or Ranged Weapon Attack, reduce the number needed to roll a Critical Hit by 2.")
    ]
}
...
let rec multifaceted : FeatDef = {
    ...
    Grants = [ Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Multifaceted", "Select a Character Trait, an additional Archetype, and four additional skills to become proficient in.") ]
}
...
let rec performer : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Performer: Instrument", "Gain Musical Instrument Proficiency, as well as the ability to enter into a state of Performative Presence in order to buff your party.")
        Power(Action, OncePerLongRest, "PassiveFeature_Banite_TacticalDiscipline" <!!> "Performative Presence", "Buff party with +1d4 to Attack Rolls, Ability Checks, or Saving Throws. Lasts until long rest.")
    ]
}
...
let rec resilient : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Resilient", "Add your Proficiency Bonus to any Saving Throws that you make, and heal for the maximum amount when healed.")
    ]
}
...
let rec restrictedTerritory : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        Power(Reaction, AtWill, "Target_MainHandAttack" <!!> "Restricted Territory: Reaction", "Gain the ability to use your Reaction to make a Melee Weapon Attack against an enemy who enters your weapon’s range.")
        Complex("Target_MainHandAttack" <!!> "Restricted Territory: Knockback", "Additionally, when an enemy enters your weapon’s range, they will be knocked back if they take damage before ending their turn.")
        Complex("Target_MainHandAttack" <!!> "Restricted Territory: Opportunity", "Furthermore, when you make an Opportunity Attack, it deals the maximum damage possible.")
    ]
}
...
let rec savageAssault : FeatDef = {
    ...
    Grants = [
        ...
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Savage Assault: Corrosive", "Additionally, when you damage a target, you reduce its AC by 1 (stacking).")
    ]
}
...
let rec sentinel : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        Power(Reaction, AtWill, "PassiveFeature_Sentinel_ZeroSpeed" <!!> "Sentinel: Vengeance", "When an enemy makes an Attack Roll against an ally, you can use your Reaction in order to make a Melee or a Ranged Weapon Attack against that enemy.")
...
let rec sharpshooter : FeatDef = {
    ...
    Grants = [
        ...
        Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Sharpshooter: Aim", "Additionally, Attack Rolls with ranged weapons do not benefit from your Proficiency Bonus. However, the bonus is added twice when rolling for damage. This secondary effect can be toggled at any time.")
...
let rec shieldbearer : FeatDef = {
    ...
    Grants = [
        ...
        Complex("PassiveFeature_MultiattackDefense" <!!> "Shieldbearer: Charges", "Furthermore, gain three Shieldbearer Charges which can be used to negate an instance of incoming damage.")
        Power(FreeAction, AtWill, "PassiveFeature_MultiattackDefense" <!!> "Shieldbearer: Deflect", "Additionally, you can spend a Shieldbearer charge to reflect projectiles for 1 turn as a Free Reaction after performing a Shield Bash.")
    ]
}
...
let rec spellbreak : FeatDef = {
    ...
    Grants = [
        ...
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Spellbreak", "When casting a Spell or Cantrip, reduce the number needed to roll a Critical hit by 2.")
    ]
}
...
let rec tormentor : FeatDef = {
    ...
    Subpicks = Map []
    Grants = [
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Tormentor", "Whenever you deal damage, you heal for 25% of that damage.")
    ]
}
...
let rec tough : FeatDef = {
    ...
    Grants = [
        ...
        yield! alsoAffectsSummons <| Complex("PassiveFeature_Banite_TacticalDiscipline" <!!> "Tough: Vitality", "Your maximum Hit Points increases by 4 for each level you have gained.")
...
let rec warCaster : FeatDef = {
    ...
    Grants = [
        ...
        Power(FreeAction, AtWill, "PassiveFeature_WarCaster_OpportunitySpell" <!!> "War Caster: Reaction", "Additionally, you can cast Shocking Grasp against an enemy who enters into melee range as a Free Reaction, as well as Bind your Weapons.")
        Power(BonusAction, AtWill, "PassiveFeature_WarCaster_OpportunitySpell" <!!> "Bind Weapons", "Binding your weapons is a Bonus Action that allows you to use your Spellcasting Ability for Attack Rolls and Damage. Once per turn, upon killing another creature or landing a Critical Hit with your Bound Weapon, you restore a level one Spell Slot.")
        Complex("PassiveFeature_WarCaster_OpportunitySpell" <!!> "War Caster: Melee Spell", "Furthermore, you may cast any ranged Spell or Cantrip as a melee Spell. This may be toggled at any time.")
    ]
}
